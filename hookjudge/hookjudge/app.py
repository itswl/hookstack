...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def metrics(
        ...
        lines += [
            ...
            "# HELP hookjudge_wake_someone The judge's second axis: does a person act now.",
            "# TYPE hookjudge_wake_someone gauge",
            f'hookjudge_wake_someone{{answer="yes"}} {attention.get("wake_yes") or 0}',
            f'hookjudge_wake_someone{{answer="no"}} {attention.get("wake_no") or 0}',
            "# HELP hookjudge_quiet_regrets wake=no rows a person later ruled mattered.",
            "# TYPE hookjudge_quiet_regrets gauge",
            f"hookjudge_quiet_regrets {attention.get('quiet_regrets') or 0}",
            "# HELP hookjudge_quiet_regret_conditions conditions quieted AND ruled mattered in the window.",
            "# TYPE hookjudge_quiet_regret_conditions gauge",
            f"hookjudge_quiet_regret_conditions {attention.get('quiet_regret_conditions') or 0}",
