...
def event(title: str = "示例充值超500告警", body: str = "用户 42 充值 920 元", **kw: Any) -> Incoming:
...
@pytest.mark.parametrize(
    "title,expected",
    [
        ("[RESOLVED] 示例充值超500告警", True),
        ("[FIRING:1] 示例充值超500告警", False),
        ("磁盘告警已恢复", True),
        ("okhttp 连接超时", False),
        ("status: OK", True),
    ],
...
def test_the_pipes_real_normalized_wire_shape_parses():
    ...
    assert parsed.title == "示例充值超500告警"
    ...
    assert parsed.identity == "grafana|示例充值超500告警|env=prod"
