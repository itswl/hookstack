...
EVENT = {
    ...
    "event": {
        ...
        "body": "用户 42 充值 920 元",
        "level": "high",
        "fields": {"project": "alarm-prod", "env": "prod"},
    },
    "raw": {"state": "alerting"},
}
...
async def test_the_judgement_is_handed_back_signed_and_correlated(app_client, monkeypatch):
    ...
    assert sent["body"]["identity"] == {"project": "alarm-prod", "env": "prod"}
...
async def test_status_and_metrics_are_guarded_and_report_the_cost_question(app_client):
    ...
    assert body["recent"][0]["title"] == "充值金额单次超500报警"
