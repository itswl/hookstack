...
| `HOOKPROBE_AUTOMATION_TIERS` | *(empty = defaults)* | The declared ceiling per class of automation, `class=tier` comma-separated over the defaults — `memory=propose,remediation=propose`. The ladder is `verdict_only < investigate < propose < auto_apply`; a class may do at most its tier, and the scattered auto-apply knobs (`HOOKPROBE_MEMORY_AUTO_APPLY`, the remediation allowlist, `HOOKPROBE_ESCALATE_LEVELS`) operate UNDER it — a knob cannot grant what the tier forbids. Defaults preserve today's behaviour, so this is inert until a line is written. **It is not what makes graduation safe** — the RECORD is: every class accumulates proposals and the human decisions on them (`GET /v1/automation`), and raising a ceiling is justified by that record, never by an argument or an agreement rate. A typo in a tier name keeps the default rather than raising it. See `automation.py` and the sampling patrol |
