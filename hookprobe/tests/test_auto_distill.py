...
def install(skills: Path, run=None, limit: int = 10, input_changes=()):
    return distill.auto_install(
        run if run is not None else run_record(), skills_dir=skills, limit=limit, input_changes=input_changes
    )
...
def test_an_existing_runbook_is_never_overwritten(skills: Path) -> None:
    """The property that keeps a bad run from erasing a good runbook."""
    first = install(skills)["installed"]
    (skills / first / "SKILL.md").write_text("reviewed by a human\n")

    outcome = install(skills)

    assert "already exists" in outcome["skipped"]
    assert (skills / first / "SKILL.md").read_text() == "reviewed by a human\n"
...
def test_the_cap_holds_and_evicts_nothing(skills: Path) -> None:
...
def test_two_investigations_of_the_same_thing_do_not_pile_up(skills: Path) -> None:
    """The name comes from the question, so a recurring alert reuses its runbook."""
    first = install(skills)
    second = install(skills, run=run_record(session_key="hookprobe:def", run_id="cafe"))

    assert "installed" in first
    assert "already exists" in second["skipped"]
