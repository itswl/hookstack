...
RESULT = {
    ...
    "identity": {"project": "demo-alarm", "env": "prod", "rule": "示例充值超限告警"},
...
def test_feishu_gets_a_card_with_every_block_and_the_buttons():
    ...
    assert "示例充值超限告警" in header["title"]["content"]
...
def test_dingtalk_and_wecom_get_their_own_markdown_without_dead_buttons():
    ...
    assert text.startswith("### 📡 示例充值超限告警"), "DingTalk wants a heading"
    ...
    assert content.startswith("**📡 示例充值超限告警**"), "WeCom renders bold, not #"
