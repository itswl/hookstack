...
RESULT = {
    ...
    "identity": {"project": "alarm-prod", "env": "prod", "rule": "充值金额单次超500报警"},
...
def test_feishu_gets_a_card_with_every_block_and_the_buttons():
    ...
    assert "充值金额单次超500报警" in header["title"]["content"]
    ...
    assert "alarm-prod · prod" in blocks, "identity reads as a breadcrumb, not a label grid"
...
def test_dingtalk_and_wecom_get_their_own_markdown_without_dead_buttons():
    ...
    assert text.startswith("### 📡 充值金额单次超500报警"), "DingTalk wants a heading"
    ...
    assert content.startswith("**📡 充值金额单次超500报警**"), "WeCom renders bold, not #"
