...
- **AWS SNS cannot be templated — closed 2026-08-12** by
  ...
  (`{AlarmName}` works), answers the SubscriptionConfirmation handshake with
  SubscribeURL pinned to SNS hosts, and verifies the certificate signatures
  when `cryptography` is installed — one loud warning and transport trust
  when it is not.
